...
- **src/proxy/btpProxy.ts** - `BtpProxy`: a facade over the above. `getAuthorizationHeader(destination)` (retried, since a refusal is not always an answer) and `getTargetUrl(destination)`, plus broker construction: `new AuthBroker({ serviceKeyStore, sessionStore, provider }, logger)` with a provider FACTORY, so the broker seeds each destination's `AuthorizationCodeProvider` with the stored refresh and access token. A configured `targetUrl` is written into the session as its `serviceUrl` (only that — auth-broker 3 refuses a destination with no `serviceUrl` anywhere, and XSUAA keys usually carry none). **No circuit breaker** — it only ever guarded the buffered axios forward, and the streaming path has nowhere to put one without buffering the response again
...
- **src/lib/authFailure.ts** - `ServiceKeyNotFoundError` (the proxy's own, thrown before the broker is asked, since auth-broker 3 reports a missing key only as a missing `serviceUrl`) and `classifyAuthFailure()`, which tells failures apart by class and `code` through the `cause` chain first and by message only where nothing typed says more. auth-broker 3 passes provider errors on unchanged — never match on a rewrapped broker message
