/**
 * MCP Proxy - Proxies requests to MCP servers with JWT authentication
 *
...
export class McpProxy {
    ...
    constructor(defaultBtpAuthBroker: AuthBroker, config?: Partial<ProxyConfig>) {
        ...
        if (config) {
            ...
            logger?.debug('Using provided config in McpProxy constructor', {
            ...
            logger?.info(
                'Using fallback config from CLI/ENV in McpProxy constructor',
                {
    ...
    public static async create(
        ...
        return new McpProxy(defaultBtpAuthBroker, loadedConfig);
    }
}

/**
 * Factory function to create McpProxy
 */
export async function createMcpProxy(
    config?: Partial<ProxyConfig>,
): Promise<McpProxy> {
    return McpProxy.create(config);
}
