...
export function parseTransportConfig(): TransportConfig {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.startsWith("--transport=")) {
      ...
      if (value === "stdio" || value === "streamable-http" || value === "http" || value === "sse") {
        transportType = value === "http" ? "streamable-http" : value as TransportType;
      }
    }
  ...
  if (!transportType && process.env.MCP_TRANSPORT) {
    ...
    if (envTransport === "stdio" || envTransport === "streamable-http" || envTransport === "http" || envTransport === "sse") {
      transportType = envTransport === "http" ? "streamable-http" : envTransport as TransportType;
    }
  }
  ...
  if (transportType === "sse") {
    ...
    return {
      ...
      host,
      port,
      allowedOrigins: parseListOption("--sse-allowed-origins", "MCP_SSE_ALLOWED_ORIGINS"),
      allowedHosts: parseListOption("--sse-allowed-hosts", "MCP_SSE_ALLOWED_HOSTS"),
  ...
  if (transportType === "streamable-http") {
    ...
    return {
      ...
      allowedOrigins: parseListOption("--http-allowed-origins", "MCP_HTTP_ALLOWED_ORIGINS"),
      allowedHosts: parseListOption("--http-allowed-hosts", "MCP_HTTP_ALLOWED_HOSTS"),
      enableDnsRebindingProtection: parseBooleanOption(
 ...
 */
function parseListOption(argName: string, envName: string): string[] | undefined {
  const argValue = getArgValue(argName);
  if (argValue) {
    return argValue.split(",").map((s) => s.trim()).filter((s) => s.length > 0);
  }
  const envValue = process.env[envName];
  if (envValue) {
