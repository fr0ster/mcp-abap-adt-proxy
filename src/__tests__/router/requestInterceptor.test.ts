...
describe("requestInterceptor", () => {
  describe("interceptRequest", () => {
    ...
    it("should extract method and URL from request", () => {
      ...
      testlogger?.info("Input request object - raw HTTP request from client", { 
        ...
        headers: {
          ...
          values: Object.entries(mockReq.headers || {}).map(([k, v]) => ({ key: k, value: v }))
        },
        socket: {
      ...
      testlogger?.info("Intercepted request result - processed request data", { 
        ...
        extractedData: {
          ...
          headerCount: intercepted.headers ? Object.keys(intercepted.headers).length : 0,
    ...
    it("should extract headers from request", () => {
      ...
      testlogger?.debug("Calling interceptRequest", {
        ...
        parameter: {
          ...
          headerCount: mockReq.headers ? Object.keys(mockReq.headers).length : 0
        }
      });
      ...
      testlogger?.debug("Intercepted result object", { 
        ...
        matches: {
          btpDestination: intercepted.headers[HEADER_BTP_DESTINATION] === "btp-cloud",
          mcpDestination: intercepted.headers[HEADER_MCP_DESTINATION] === "sap-abap"
        }
      });
    ...
    it("should pass config overrides to header analyzer", () => {
      ...
      testlogger?.debug("Intercepted result with routing decision", { 
        ...
        matches: {
          btpDestination: intercepted.routingDecision.btpDestination === "cli-btp",
          mcpDestination: intercepted.routingDecision.mcpDestination === "cli-mcp",
          overridesApplied: intercepted.routingDecision.btpDestination !== mockReq.headers?.[HEADER_BTP_DESTINATION]
        }
      });
  ...
  describe("sanitizeHeadersForLogging", () => {
    ...
    it("should handle array values by joining", () => {
      ...
      expect(sanitized[HEADER_MCP_URL]).toBe("https://example.com, https://other.com");
    });
