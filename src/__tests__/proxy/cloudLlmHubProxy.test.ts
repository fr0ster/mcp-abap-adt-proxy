...
describe('CloudLlmHubProxy', () => {
  ...
  describe('buildProxyRequest', () => {
    it('should require btpDestination and add Authorization header', async () => {
      ...
      testLogger?.info(
        'Mock configuration - simulating service key and token retrieval',
        {
          ...
          mockConnectionConfig,
          explanation:
            'Mocking AuthBroker.getToken() and getConnectionConfig() calls that would normally read from service key store',
        },
      );
      ...
      testLogger?.info('Mock method calls executed', {
        ...
        getConnectionConfig: {
          ...
          calls: mockBtpAuthBroker.getConnectionConfig.mock.calls.map(
            (call, idx) => ({
              callNumber: idx + 1,
              destination: call[0],
              expected: 'btp-cloud',
      ...
      testLogger?.info('All assertions passed', {
        ...
        url:
          config.url === `${mockConnectionConfig.serviceUrl}/mcp/stream/http`,
        tokenRetrieved: true,
    ...
    it('should add SAP headers from x-mcp-destination', async () => {
      ...
      mockAbapAuthBroker.getConnectionConfig.mockResolvedValue(
      ...
      testLogger?.debug('Mock method calls', {
        ...
        abapBroker: {
          ...
          getConnectionConfig: {
            called:
              mockAbapAuthBroker.getConnectionConfig.mock.calls.length > 0,
      ...
      expect(mockAbapAuthBroker.getConnectionConfig).toHaveBeenCalledWith(
        'sap-abap',
      );
      ...
      testLogger?.debug('Assertions passed', {
        ...
        sapUrl:
          config.headers[HEADER_SAP_URL] === abapConnectionConfig.serviceUrl,
    ...
    it('should work with only mcpDestination (local testing mode)', async () => {
      ...
      (mockAbapAuthBroker.getConnectionConfig as jest.Mock).mockResolvedValue({
        serviceUrl: 'https://sap.example.com',
        authorizationToken: '',
      });
